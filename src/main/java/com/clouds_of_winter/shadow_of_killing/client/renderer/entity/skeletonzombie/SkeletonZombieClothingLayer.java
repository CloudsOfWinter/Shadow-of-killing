...
@OnlyIn(Dist.CLIENT)
public class SkeletonZombieClothingLayer<T extends Mob & RangedAttackMob, M extends EntityModel<T>> extends RenderLayer<T, M> {
    private static final ResourceLocation SKELETON_ZOMBIE_CLOTHES_LOCATION = new ResourceLocation(ShadowofKilling.MOD_ID, "textures/entity/skeleton_zombie/skeleton_zombie_overlay.png");
    private final SkeletonModel<T> layerModel;

    public SkeletonZombieClothingLayer(RenderLayerParent<T, M> p_174544_, EntityModelSet p_174545_) {
        super(p_174544_);
    ...
    public void render(PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, T entity, float limbSwing, float limbSwingAmount, float partialTick, float ageInTicks, float netHeadYaw, float headPitch) {
        if (entity.isInvisible()) {
            ...
            coloredCutoutModelCopyLayerRender(this.getParentModel(), this.layerModel, SKELETON_ZOMBIE_CLOTHES_LOCATION, poseStack, bufferSource, packedLight, entity, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, partialTick, 1.0F, 1.0F, 1.0F
            );
        }
    }
    private void renderLayerWithAlpha(PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, T entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float partialTick, float alpha) {
        VertexConsumer vertexconsumer = bufferSource.getBuffer(RenderType.entityTranslucent(SKELETON_ZOMBIE_CLOTHES_LOCATION));
