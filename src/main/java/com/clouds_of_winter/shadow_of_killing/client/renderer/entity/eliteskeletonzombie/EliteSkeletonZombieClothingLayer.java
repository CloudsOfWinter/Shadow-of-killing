...
@OnlyIn(Dist.CLIENT)
public class EliteSkeletonZombieClothingLayer<T extends Mob & RangedAttackMob, M extends EntityModel<T>> extends RenderLayer<T, M> {
    private static final ResourceLocation ELITE_SKELETON_ZOMBIE_CLOTHES_LOCATION = new ResourceLocation(ShadowofKilling.MOD_ID, "textures/entity/elite_skeleton_zombie/elite_skeleton_zombie_overlay.png");
    private final SkeletonModel<T> layerModel;

    public EliteSkeletonZombieClothingLayer(RenderLayerParent<T, M> p_174544_, EntityModelSet p_174545_) {
        super(p_174544_);
    ...
    public void render(PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, T entity, float limbSwing, float limbSwingAmount, float partialTick, float ageInTicks, float netHeadYaw, float headPitch) {
        if (entity.isInvisible()) {
            ...
            coloredCutoutModelCopyLayerRender(this.getParentModel(), this.layerModel, ELITE_SKELETON_ZOMBIE_CLOTHES_LOCATION, poseStack, bufferSource, packedLight, entity, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, partialTick, 1.0F, 1.0F, 1.0F
            );
        }
    }
    private void renderLayerWithAlpha(PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, T entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float partialTick, float alpha) {
        VertexConsumer vertexconsumer = bufferSource.getBuffer(RenderType.entityTranslucent(ELITE_SKELETON_ZOMBIE_CLOTHES_LOCATION));
